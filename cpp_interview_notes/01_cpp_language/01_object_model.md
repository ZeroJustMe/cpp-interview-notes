...
### 更深入的理解
为什么说"通常"而不是"必须"？因为 C++ 标准定义了语义，但没有强制规定所有 ABI 的具体物理布局。你在面试里常说的 `vptr`、`vtable`、对象首地址放虚表指针，这些更多是主流实现（如 GCC/MSVC/Clang 对应 ABI）的典型做法，而不是标准原文写死的唯一实现方式。
...
不冲突。空类作为独立对象时通常有大小 1；但当它作为**基类子对象**时，编译器可以利用空基类优化，让空基类不额外占空间。
...
- **静态成员变量**属于类本身，所有对象共享一份，不计入 `sizeof`
...
### RTTI 是什么？
RTTI（Run-Time Type Information）是运行时类型信息机制，`typeid` 和 `dynamic_cast` 是与 RTTI 密切相关的典型语言工具。真正和动态类型强相关的 RTTI 通常依赖多态类型。
